---
name: session-open
description: Runs first in every session of a copy of this template: read CLAUDE.md + the volatile slice, check commits/peers, report the orientation block, write nothing.
---
...
## 6. Step four - check for live peer sessions

When the harness this repository runs on offers peer messaging, orientation
includes this step. The full procedure - listing live peers, asking each its
status in one line, and never inferring a peer's state from `HANDOFF.md`
instead of a fresh answer - is the multi-session skill's rule; invoke it (or
read its reference) for the complete "status is asked, never asserted"
mechanics. Where the harness offers no such messaging, skip this step and say
so rather than silently omitting it.

Arm the resume guard: skill:resume-guard (one-shot cron at the next window boundary).

## 7. Step five - locate a live thread
...
## 8. Step six - report the orientation block
...
## 10. When the signals do not agree
...
## 11. What this ritual never does

- Never writes a file, a row, or a placeholder before the halt in step seven.
...
Source: .claude/skills/session-open/SKILL.md frontmatter §1-11 (moved into this skill 2026-09-16)
